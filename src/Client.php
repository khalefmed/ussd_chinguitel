    ...
    public function getTransport()
    {
        if (empty($this->transport)) { 
           $hosts = $ports = [];
            ...
            $maxAttempts = 10000; // Maximum number of connection attempts
            $attempts = 0;
            while ($attempts < $maxAttempts) {
            try {
            $this->transport = new SMPPSocketTransport($hosts, $ports, false, $this->debugHandler);
            $this->transport->setRecvTimeout(300000000000000000000000); // 10 seconds
            break;
           } catch (Exception $e) {
                // Handle the exception here, for example, log it.
                // You can also introduce a delay between retries if needed.
                $attempts++;
            }
        }
           if ($attempts === $maxAttempts) {
            // Handle the case where maximum connection attempts are reached.
            // You can throw an exception or take other appropriate action.
            throw new Exception("Failed to establish a connection after $maxAttempts attempts.");
        }
    ...
    public function listenSm(callable $callback)
    {
        do { echo"-------------------------------------------------------------------------\n";
            ...
            if (PduParser::isSm($pdu)) {
                try {
                    $sm = PduParser::fromPdu($pdu);echo"FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF ";
...
if ($response === false) {
    ...
    throw new SmppException('Failed to send command: ' . SMPP::SUBMIT_SM);
}


	echo "ha4i repons : ". $response->body;
	echo "ha4a pdu: $pdu";
	echo "ha4a need reply: $needReply";
        if ($needReply) {
	...
	if ($needReply) {
            ...
            try{
                ...
                $s=$response->body;
                $a=SMPP::ESME_ROK;
                echo "\nid pud ".$pdu->id; echo "body ".$pduBody; echo "status : ".$response->status."\n";
           /* if ($response === false) {
                throw new SmppException('Failed to read reply to command: 0x' . dechex($id));
                }*/

               /* if ($response->status != SMPP::ESME_ROK) { echo "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$";
                     throw new SmppException(SMPP::getStatusMessage($response->status), $response->status);
	       }*/
            }catch (Exception $e) {
    ...
    )
    {
        ...
        if (!empty($tags)) {
            foreach ($tags as $tag) {
                $pdu .= $tag->getBinary(); echo $tag->value;
            }
        }
        if ($this->debug) {
